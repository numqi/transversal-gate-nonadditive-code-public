...
from utils import hfb, SpecialUnitary2XZManifold

from zzz233 import to_pickle_wrapper, from_pickle_wrapper
to_pickle = to_pickle_wrapper('723_group_range.pkl')
from_pickle = from_pickle_wrapper('723_group_range.pkl')

if torch.get_num_threads() != 1:
    torch.set_num_threads(1)


class QECC723TransversalGroupModel(torch.nn.Module):
    def __init__(self, logicalX:str, group:str='BD12'):
        super().__init__()
        assert isinstance(logicalX, str) and (len(logicalX) == 7) and (set(logicalX)<={'I','X'})
        num_qubit = 7
...
def hf_task(lambda2_list:np.ndarray, logicalX:str, trans_group:str, kwargs0:dict, kwargs1:dict):
    model = QECC723TransversalGroupModel(logicalX, group=trans_group)
    z0 = []
...
if __name__=='__main__':
    ...
    logicalX_list = ['XXXXXXX', 'XXXXXII']
    lambda2_list = np.linspace(0, 7.5, 101)
...
def dump_plot_data():
    ...
    group_list += '2T BD6 BD8 BD10'.split(' ')
    logicalX_list = ['X'*x+'I'*(7-x) for x in range(1,8)][::-1]
    with open('723_group_range.pkl', 'rb') as fid:
...
def hf_plot():
    ...
    ax.set_ylabel('loss')
    ax.set_title(f'723 {group} logicalX={logicalX}')
    fig.tight_layout()
...
def hf233():
    ...
    a = np_rng.uniform(0, np.sqrt(8))
    sign = np.ones(9, dtype=np.float64)
    code,info = numqi.qec.q723.get_BD32(a, sign, return_info=True)
    error_str,_ = numqi.qec.make_pauli_error_list_sparse(7, distance=3, kind='scipy-csr01')
    ind_list = [
        [29,110,146,173], #ZZIIIII IZIIIZI IIZIIZI IIIZIZI Z1Z2 Z2Z6 Z3Z6 Z4Z6
        [65,74,209], #ZIIIIZI ZIIIIIZ IIIIIZZ Z1Z6 Z1Z7 Z6Z7
        [56,191], #ZIIIZII IIIIZZI Z1Z5 Z5Z6
    ]
    for x in ind_list:
        tmp0 = [error_str[i] for i in x]
        tmp1 = [''.join(f'{y}{i+1}' for i,y in enumerate(x) if y!='I') for x in tmp0]
        print(x, ' '.join(tmp0), ' '.join(tmp1))
